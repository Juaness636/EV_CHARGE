...
export async function listarReportesAdmin(): Promise<unknown[]> {
...
export async function listarCalificacionesAdmin(): Promise<unknown[]> {
  const { data } = await api.get('/admin/calificaciones');
  return data;
}
