      ...
      <nav id="navbar">
        <div className="nav-inner">
          ...
          <ul className={`nav-links${mobileMenuOpen ? ' open' : ''}`}>
            ...
            <li><a href="#servicios" className="nav-link" onClick={() => setMobileMenuOpen(false)}>Categoría</a></li>
          ...
          <div className="auth-card">
            ...
            {authTab !== 'recuperar' && (
              <div className="auth-tabs">
                ...
                <button className={`auth-tab${authTab === 'registro' ? ' active' : ''}`} onClick={() => switchAuthTab('registro')}>
                  Registrarse
                </button>
              </div>
            )}
            ...
            {authTab === 'login' && (
              <form id="form-login" onSubmit={doLogin}>
                <div className="form-group">
                  <label>Correo</label>
                  <input type="email" id="login-email" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} placeholder="correo@ejemplo.com" />
                </div>
                <div className="form-group">
                  <label>Contraseña</label>
                  <div className="password-box">
                    <input id="login-pass" type={showLoginPass ? 'text' : 'password'} value={loginPass} onChange={(e) => setLoginPass(e.target.value)} placeholder="••••••••" />
                    <span className="password-eye" onClick={() => setShowLoginPass((value) => !value)}>
            ...
            {authTab === 'registro' && (
              <form id="form-registro" onSubmit={doRegistro}>
                ...
                <div className="form-group"><label>Contraseña</label><div className="password-box"><input type={showRegPass ? 'text' : 'password'} value={regPass} onChange={(e) => setRegPass(e.target.value)} placeholder="Crea tu contraseña" /><span className="password-eye" onClick={() => setShowRegPass((value) => !value)}><i className={`fa-solid ${showRegPass ? 'fa-eye-slash' : 'fa-eye'}`}></i></span></div><div className={`strength-bar${regPass.length ? ' show' : ''}`}><div className="strength-progress" style={{ width: `${passwordPercent}%`, background: passwordPercent < 40 ? '#e74c3c' : passwordPercent < 80 ? '#f39c12' : '#39a900' }}></div></div><ul className={`password-requisitos${regPass.length ? ' show' : ''}`}>{requirementsList.map(([key, text]) => <li key={key} className={requirements[key] ? 'ok' : ''}><i className={`fa-solid ${requirements[key] ? 'fa-check' : 'fa-xmark'}`}></i> {text}</li>)}</ul></div>
                <div className="form-group"><label>Confirmar contraseña</label><input type="password" value={regPassConfirm} onChange={(e) => setRegPassConfirm(e.target.value)} placeholder="Repite tu contraseña" onPaste={(e) => e.preventDefault()} onCopy={(e) => e.preventDefault()} onCut={(e) => e.preventDefault()} autoComplete="off" /></div>
                <button id="btn-crear-cuenta" className="btn btn-primary btn-block" disabled={!passwordComplete || authLoading}>{authLoading ? 'Creando...' : 'Crear cuenta'}</button>
              </form>
            )}
